...
## The analytics run on Synthea, and it found something about *our* data
...
generator emits exactly the five categories the analytics expect, at exactly the
grain they assume.

[Synthea](https://github.com/synthetichealth/synthea) is free, is not written by
me, and drives utilisation from clinical modules rather than a rate table.

```bash
python run_synthea.py     # reads the population generated by ../data2-fhir-hedis
```

### 1. The category taxonomy has holes

Synthea emits ten encounter classes; this project has five categories. **Skilled
nursing, hospice and home health have no bucket at all** — about 4% of spend.

Folding them into `inpatient` is the tempting fix and it is measurably wrong:
...
More than a twentyfold spread. A single merged average describes none of them.
So `CATEGORY_MAP` is explicit and **deliberately incomplete** — unmapped classes
keep their own names and are reported separately, putting the hole in the output
instead of hiding it in an average.

*(These multiples move with the reporting window — over 2020–2024 SNF came out
at 1.48× and hospice at 1.27×. `run_synthea.py` computes the table rather than
the prose asserting numbers that drift.)*

### 2. Our own generator has a light tail — the bigger finding

Cost concentration is the number a care-management programme is sized on, and
this README argues for reporting it **precisely because the distribution is
skewed**. Measured against Synthea, ours is not:

| | Synthea | own generator |
|---|---|---|
| top 1% share of spend | **13.3%** | 4.8% |
| top 5% share of spend | **43.0%** | 18.4% |
| p99 / mean member spend | **10.6×** | 4.1× |

**This is structural, not bad luck.** `synth.generate` draws each category as
`Poisson(rate × risk_score × covered_days)` and prices it with a bounded uniform
`U(0.55, 1.6)`. The only between-member driver is `risk_score`, which spans
**0.60 to 2.08 — a range of 3.5×**.

No member *can* be fifty times the mean, because nothing in the model lets them
be. Real books have one transplant, one long NICU stay, one haemophilia patient
— and those single members move a PMPM.

**What that means for the analytics built on it:** `cost_concentration` is
correct code measured on an unrepresentative population. Anything sized off the
top-N% — outreach capacity, stop-loss attachment, high-cost-claimant review —
would be sized wrong. The generator remains right for what it was built for,
planting a rate and recovering it, and wrong for anything depending on the shape
of the tail. That limitation was not previously written down anywhere.
...
- **Synthea is now used as a second source, and it exposed a limit in the
  first one** (see above). `src/synth.py` remains the default because a planted
  rate is what makes the analytics checkable — but its tail is too light for
  anything sized off cost concentration, and that is now written down.
...
| `run_synthea.py` | the comparison; found the light-tail limitation |
| `tests/test_synthea_claims.py` | 9 tests; 4 need no Synthea |
